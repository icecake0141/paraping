...
when the buffer is full, so memory is strictly bounded regardless of uptime.
Snapshots are copied to avoid live-state mutation in history view.
...
メモリは厳密に制限されます。履歴表示のためにスナップショットはコピーされ、ライブ状態の
変更が過去データに波及しません。
